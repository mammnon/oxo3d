package fr.thomas.control.servlets;

import java.io.IOException;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import fr.thomas.control.utils.OponentChoiceUtils;
import fr.thomas.control.utils.PlayerUtils;

/**
 * Servlet implementation class SendInvitation
 */
@WebServlet("/SendInvitation")
public class SendInvitation extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SendInvitation() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String id = PlayerUtils.reuseOrCreateCookie(request, response);
		System.err.println("Send invitation"+id);
		OponentChoiceUtils.createInvitationEntry(id,(String)request.getParameter("oponentID"));

	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		String id = PlayerUtils.reuseOrCreateCookie(request, response);
		System.err.println("Send invitation"+id);
		OponentChoiceUtils.createInvitationEntry(id,(String)request.getParameter("oponentID"));
	}

}
